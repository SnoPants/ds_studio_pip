...
See [ROADMAP.md](./ROADMAP.md) for current scope, implementation status, and the decision to use the skeleton itself as the rig guide. The proposed structure, orientation design, schema example, and development-phase notes below describe earlier planning; the roadmap takes precedence where they differ.
